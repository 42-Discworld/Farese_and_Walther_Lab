####################################################################################
# Script: FWL_FattyAcids_Length_1.2.R
# Author: Wenting Lyu
...
length_info <- normalized_length %>% ungroup() %>% cal_lipid_statistics(., group_info, method, by_group)
length_info_long <- length_info[[2]] 
if(method == "mean"){
  length_sd <- normalized_length %>% ungroup() %>% cal_lipid_statistics(., group_info, "sd", by_group)
...
fas_ctr <- fas_class %>% ungroup() %>% cal_lipid_statistics(., group_info, method, par_group)
fas_ctr_gr <- fas_ctr[[1]] %>% select(all_of(par_group), all_of(control_nm))
fas_all <- left_join(fas_class, fas_ctr_gr)  
filtered_fas <- fas_all %>% filter(eval(!!sym(control_nm)) > 0)
fas_nm <- norm_by_mean2(filtered_fas, par_group, sample_raw_list, control_nm, "n")
exclude_fas <- fas_all %>% 
  filter(eval(!!sym(control_nm)) <= 0) %>% 
  select(Class, FA) %>% 
...
if(method == "mean"){
  for(i in classes){
    ...
    name <- paste0(i, " Relative fold change (Normalized by ", method, ")")
    p <-  ggbarplot(fa_chain, x = "FA", y = "VALs",
                    fill = "GROUPS", width = 0.6,
