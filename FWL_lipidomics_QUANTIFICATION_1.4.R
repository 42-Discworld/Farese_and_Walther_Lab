...
for(i in 1:fc_times){
  ...
  molec_info <- molec_nm %>% ungroup() %>% cal_lipid_statistics(., group_info, method, group_bar)
  molec_info_long <- molec_info[[2]] 
  gtitle <- paste0("Normalized by (", method, ")")
  if(method == "mean"){
    molec_sd <- molec_nm %>% ungroup() %>% cal_lipid_statistics(., group_info, "sd", group_bar)
