...
Suggested using homebrew for installing XQaurtz and gfortran.
  ...
  - Installing gfortran: `brew install
gcc`
...
<img src="Manual_of_Lipidomics_pipeline_files/figure-gfm/unnamed-chunk-1-1.png" width="50%" />
  ...
  - To run the pipeline:
      ...
      - Second, after clicking R project, run script
        ...
        *`source("txt_to_csv_version.R")`* in the console. User could
        also go to script and run the command line by line. Please note
...
**(7) Input group information**
  ...
  - Overview: User needs to input group information, i.e. experiment
    group names and sample index numbers. Pipeline will extract sample
    AUC’s from corresponding columns information. Group information will
    be stored in `group_information.csv`.
      ...
      - Pipeline will ask if user wants to edit group information for
  ...
  - Code display: The variable `label` in the code will be the suffix in
    ...
    subsequent analyses may be edited if user types Y in command line.
...
#### **Background Subtraction**
  ...
  - Overview: Background subtraction or NOT will process the filtered
    data. If user chooses to continue with background subtraction, the
    data will subtract the sample area from background/solvent run for
    later analysis. Previously filtered data will be stored in
    *filtered\_lipidomics\_copy* variable and `rm_duplicates.csv` in
    previous step. The newly filtered data will be under
    `subtracted_lipids.csv`. After background subtraction, the pipeline
    will provide 2 optional methods for fixing potential invalid lipid
    molecules. Method 1 will automatically delete lipid molecules in
    which the AUC are all negative or 0. Method 2 will ask the user to
    delete potentially invalid lipids in file `checkInvalid.csv`
    manually. Please note that `invalid.csv` is its copy and can be used
    as reference for user. Console then will then direct user to
    continue or not.
  - Utility:
      - Due to limitations of the machine and various other. Data could
        contain 0 or negative values. Pipeline will detect potentially
        invalid values and calculate their percentage information of the
        sample size. Please check files `neg.percent.csv`,
        `checkInvalid.csv` or `invalid.csv`, `imputeNA.csv`.  
      - type 1 or 2  
  - Example display
