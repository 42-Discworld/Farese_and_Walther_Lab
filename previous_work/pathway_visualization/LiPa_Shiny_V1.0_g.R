...
Gmap=read.delim(file.choose(), header=TRUE)
...
##PLOTs
a<-mybarplot_all(rec)
b<-mybarplot_all(Satrec)
c<-mybarplot_all(Sat_Srec)
d<-mybarplot_all(Ratios4)
e<-mybarplot_all(ABSpmol_rec)
f<-pmolplot(pmol_rec)
g<-mybarplot_Individual(recIND)


#Lipa Plot Viewer
ima <- readPNG(file.choose("lipid_map.png"))
lipa<-ggplot(Graphic4, aes(X, Y),na.rm = TRUE)+
...
plotType <-function(type){
  ...
  Ratio_Normalized=d,
  pmol_Normalized=e,
  pmol_as_perc_of_total=f,
  Lipa=lipa
)
}
runApp(list(
  ui = bootstrapPage(
    radioButtons("pType", "Choose plot type for lipids:",
                 list("Area_Normalized", "Double_saturation","Single_saturation",
                      "Ratio_Normalized", "pmol_Normalized", "pmol_as_perc_of_total", "Lipa")),
    plotOutput('plot')
  ),
